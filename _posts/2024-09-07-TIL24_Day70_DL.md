...
# Lecture 5: Transformers and Foundation Models (1)
...
5. **Feed-Forward Network (FFN)**:
   - After applying self-attention, each encoder and decoder block has a **feed-forward network**, which is applied to each position independently. The FFN typically consists of two linear layers with a ReLU activation in between, allowing for non-linear transformations of the attention outputs.<br><br>
