...
<img src="/blog/images/2024-08-19-TIL24_Day56/image-20240826155432591.png" width="60%">
...
In the code, we initialized the `StratifiedKFold` iterator, passing in the `y_train` class labels from the training dataset, and specifying the number of folds using the `n_splits`parameter. Throughout each iteration, we ensured that the examples were appropriately scaled, such as standardizing them, using the `pipe_lr` pipeline. Subsequently, we used the `test` indices to compute the accuracy score of the model, gathering the `scores` in a list to later calculate the average accuracy and the standard deviation of the estimate.
