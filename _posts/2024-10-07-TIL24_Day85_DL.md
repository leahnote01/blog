...
- Token Vocabulary
  ...
  > **Token vocabulary** in large language models (LLMs) refers to the set of tokens that the model can recognize and use as input and output during its training and inference processes. Depending on how the model's tokenizer is configured, a token can be a word, subword, or character. 

  - We want to be able to generate arbitrary texts.
  - Our vocabulary size specifies the output layer size.
    - If we have a vast vocabulary, we will have a significant output layer size.
...
![image-20241008143719351](/images/2024-10-07-TIL24_Day85_DL/image-20241008143719351.png)
