...
- Token Vocabulary
  ...
  > **Token vocabulary** in large language models (LLMs) refers to the set of tokens the model can recognize and use as input and output during training and inference processes. A token can be a word, subword, or character depending on how the model's tokenizer is configured. 
