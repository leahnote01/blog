...
- Different GPUs support different precisions. Not all GPUs support bfloat16 or TF32. Some GPUs support special operations to enable mixed-precision training to be faster. 
...
- FFCV 
  ...
  - In PyTorch, for example, FFCV can be used as an alternative to traditional data loaders like `torch.utils.data.DataLoader`. By converting datasets into FFCV's format and using FFCV's efficient data loader, training times can be significantly reduced, particularly when dealing with image datasets like **ImageNet**.<br><br><br>
...
> **Distributed Model Training** is a method in machine learning in which a model's training is <u>split across multiple machines or devices</u>. This enables **faster** and more **scalable** training, especially for large datasets and models.
...
- **Distributed operations** are fundamental communication patterns in parallel computing and distributed systems to manage data among multiple processes or nodes efficiently.
...
![image-20241010100459715](/images/2024-10-10-TIL24_Day88_DL/image-20241010100459715.png) <br><br>
...
- The rest of the memory is consumed by optimizer states,
  storing gradients, and storing activations.
- Temporary buffers used for distributed communication also
  consume memory.
...
- **Data Parallel vs FSDP**
  - Data parallel training: Each worker processes a separate batch and gradients are summed across workers using an all-reduce operation.
    - <u>This means each model keeps a copy of the weights and</u>
      <u>the optimizer states</u>, so the amount of VRAM for model
      and optimizer is 2-3x the size of the model.
