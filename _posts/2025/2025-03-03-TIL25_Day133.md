...
- In *R*, we use `glm()` function with `family=binomial` to fit logistic regression.
...
- In *Python*, we use `LogisticRegression` from `sklearn.linear_model`.
...
There are two main differences between using two programs.

- R automatically applies **reference encoding** to categorical variables, while **scikit-learn requires one-hot encoding** (`pd.get_dummies()`).
- **Regularization**: `scikit-learn` applies **L2 regularization by default**, but we disable it using `C=1e42`. <br><br>
